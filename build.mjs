...
function para(lines) {
  const body = lines
    .filter(Boolean)
    .map((l) => esc(l).replace(/ {2,}/g, '<i class="gap"></i>'))
    .join('<br>');
...
function loadArticles() {
  ...
  const list = fs
    ...
    .map((f) => {
      ...
      return { ...meta, body: m[2].trim(), file: f };
    })
...
function footer() {
  ...
  <div class="wrap foot__in">
    ...
    <div class="foot__cols">
      <div><h4>上課時間</h4><p>${SITE.time}</p></div>
      <div><h4>上課地點</h4><p>${SITE.place}</p></div>
...
<a class="card" href="/writings/${a.slug}/">
  ...
  <div class="card__in">
    ...
    <p class="card__x">${esc(a.excerpt).replace(/ {2,}/g, '<i class="gap"></i>')}</p>
...
${hero({
  ...
  kicker: '台北 · 文山 · 萬壽橋下',
  title: '鷹捷詠春',
  sub: '黃英哲師父親授　—　以實用為目的，重視傳統功力訓練，<br>授予完整的詠春觀念。',
...
<section class="band">
  <div class="wrap band__in">
    ...
    <p class="band__s">練拳是要符合身體的運作。<br>教法著重詠春的基本觀念以及身體上的開發，並根據學生的個性及身材給予不同的調整。</p>
  </div>
</section>
  ...
  const scenes = [
    {
      ...
      title: '哪些才是真功夫？',
      lines: [
        '師父黃英哲以實用為目的，重視傳統功力訓練，',
        '授予完整的詠春觀念，',
        '並且透過實戰練習讓功夫不只是神祕的蹲馬步、打套路，',
        '而是真真實實可以保護自己跟家人的技能。',
      ],
      more: '關於師父',
    },
    ...
    },
    {
      ...
      pos: 'center 50%',
      title: '武道若夢',
      lines: ['這些都是隨意寫寫的<i class="gap"></i>跟自己徒弟說說心情而已......'],
      more: '師父手記',
    },
  ];
  const body = `
<div class="intro">
  ...
  ${scenes
    ...
    <div class="scene__in">
      ...
      <p class="scene__x">${v.lines.join('<br>')}</p>
  ...
  <section class="scene scene--end">
    <div class="scene__in">
      <p class="kicker kicker--dark">鷹捷詠春</p>
      <h2 class="scene__t scene__t--dark">師父黃英哲親授</h2>
      <p class="scene__x scene__x--dark">師承 葉問 — 講手王黃淳樑 — 林海龍</p>
      <div class="btns">
        <a class="btn btn--solid" href="/home/">進入網站</a>
        <a class="btn btn--ink" href="${SITE.fb}" target="_blank" rel="noopener">聯絡師父</a>
      </div>
    </div>
  </section>
</div>`;
...
function pageAbout() {
  const body = `
${hero({ img: 'sifu-form.jpg', kicker: '關於師父', title: '黃英哲', sub: '師承葉問—黃淳樑—林海龍一脈', pos: 'center 45%' })}
...
function pageClasses() {
  ...
  const body = `
${hero({ img: 'team-01.jpg', kicker: '課程', title: '每週二、五　橋下見', sub: '上課以實戰對練方式進行，讓觀念、功力同時進步。', pos: 'center 55%' })}
...
<section class="sec sec--paper2">
  <div class="wrap">
    ...
    <h2 class="sec__t sec__t--dark">套路、手法、步法、結構、刀棍</h2>
    <div class="course">
      ${items
        .map(
...
function pageWritings(arts) {
  const body = `
${hero({ img: 'bridge-empty.jpg', kicker: '師父手記', title: '武道若夢', sub: '這些都是隨意寫寫的　跟自己徒弟說說心情而已......', pos: 'center 55%' })}
<section class="sec sec--paper2">
  <div class="wrap">
...
<article class="art">
  ...
  ${a.quote ? `<blockquote class="pull"><p>${esc(a.quote).replace(/ {2,}/g, '<i class="gap"></i>')}</p></blockquote>` : ''}
